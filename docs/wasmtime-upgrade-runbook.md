...
| 5a | **Run the hostile-guest suite.** An upgrade changes trap behaviour under adversarial input too | `cargo test -p qqq-host --test hostile_guests` | A case that used to trap no longer traps, or traps differently |
| 5b | **Replay the fuzz corpus.** It is a *separate workspace*, so no `-p` flag reaches it; and it is the check that catches a parser upgrade changing which inputs are accepted | `cd fuzz && cargo test` (7 tests) — or `cargo run --example corpus_report` for a per-input PARSE/REJECT listing | An input that used to be refused now parses, or the corpus counts move |
