...
- [ ] **SRV-020** Implement request-body size and count limits enforced per tenant, with metrics.
  ...
  → **Per-tenant limits exist and are not yet applied**: `crates/qqq-serve/src/limits.rs` adds the **body size** cap and the **request count** cap `SRV-020` names — enforced rather than counted, in a separate type from `metrics.rs` because a metric that is also the enforcement point cannot be sampled or shipped elsewhere without silently changing what the server allows.
  → **`None` means unlimited and `Some(0)` refuses everything**, deliberately different values: a limit of zero is almost never what an operator meant, and making them distinguishable turns that mistake into a visible violation rather than a server that serves nobody (`§O-128`).
