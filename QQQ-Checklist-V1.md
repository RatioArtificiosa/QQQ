...
- [ ] **SRV-009** Implement WebSockets over HTTP/1.1 and HTTP/2.
  ...
  → **Measured**: 24 handshake tests + 30 frame tests; workspace **2029 passed, 0 failed**.
  ...
  → **Still to build**: wiring the handshake and the frame loop into `serve_connection`, and message assembly across fragments (the decoder handles one frame; a `Message` that accumulates them is the next piece). `SRV-009` stays unticked until both exist.
