...
def self_test() -> int:
    ...
    try:
        ...
        if patched == proposal:
            ...
            PROPOSAL.write_text(patched, encoding="utf-8")
            code, out = run_check()
    ...
    try:
        ...
        if patched == obs:
            ...
            OBSERVATIONS.write_text(patched, encoding="utf-8")
            code, out = run_check()
            ok = code != 0 and "parity" in out
        ...
        OBSERVATIONS.write_text(obs, encoding="utf-8")

    # The splice markers going missing: the generator must refuse rather than write a
    # file with no table in it.
    try:
        TARGET.write_text(original.replace("<!-- GENERATED:BEGIN -->", "<!-- gone -->"), encoding="utf-8")
        code, out = run_check()
