...
pub enum ErrorCode {
    ...
    /// form of "the invocation is wrong", which a caller resolves by reading the
    /// usage rather than by fixing a value. Folding the two together would make
    /// `qqqai doctor --jsonn` indistinguishable from `qqqai serve --port abc` in
    /// a log, and they need different remediation text.
    ///
    ...
    /// was implemented, and every command accepted a mistyped `--jsonn` the same
    /// way — a user who believes an option took effect, and a *missing* effect
    /// that reads as a mystery rather than an error.
    ///
    /// **Remediation:** the error names the flag; `qqqai <command> --help` lists
    /// the ones the command takes.
    CliFlagUnknown = 7004,
}
