...
- [ ] **SRV-009** Implement WebSockets over HTTP/1.1 and HTTP/2.
  ...
  → **Still to build**: wiring the handshake and this loop into `serve_connection`. `SRV-009` stays unticked until that exists — the three layers (§4, §5, assembly) are each complete and tested, and none is reachable from `serve` yet.
