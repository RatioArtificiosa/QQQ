...
- [x] **SRV-019** Implement CORS configuration with safe defaults.
  ...
  → **Still not wired**: `serve_connection` does not *apply* the CORS headers yet. The policy is built and the routes are matched; the emission point is the remaining step, and it is the same interface change `SRV-004`/`SRV-009`/`SRV-010` wait on (`Handler` gaining a non-buffered shape). Named here rather than left for a reader to infer.
