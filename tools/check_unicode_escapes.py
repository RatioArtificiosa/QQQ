...
# `\u` plus exactly four hex digits, as text.
...
def scan(path: Path) -> list[tuple[int, str]]:
    """Return (line number, the offending line) for each `\\uXXXX` in the file."""
...
def run(args) -> int:
    docs = documents()
    if not docs:
        print("FAIL -- no Markdown documents found to scan; a scan of nothing certifies nothing")
        return 1
    ...
    if total:
        ...
        return 1
    print("UNICODE ESCAPES OK -- no document contains the text of a character escape")
    return 0
...
def self_test(args) -> int:
    ...
    for label, text, want_hit in cases:
        got_hit = bool(ESCAPE.search(text))
        ok = got_hit == want_hit
        if not ok:
            failures += 1
        print(f"  {'OK  ' if ok else 'FAIL'} {label}: {'fires' if got_hit else 'silent'}")

    # Refusing vacuity must itself be testable: the document set must be non-empty.
    n = len(documents())
    ok = n > 0
    if not ok:
        failures += 1
    print(f"  {'OK  ' if ok else 'FAIL'} the corpus is non-empty ({n} document(s))")
