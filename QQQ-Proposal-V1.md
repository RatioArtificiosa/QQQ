...
**The measurement that changes the model: a fixed ~900 ns cost per crossing.** The `u64` row is the smallest possible crossing and costs ~900 ns at p50; the string row costs the same ~900 ns for 64 bytes. So the **fixed** cost of entering and leaving a component is of the order of **900 ns**, which the estimates did not price at all. A design that assumed a scalar crossing was free would be wrong by three orders of magnitude.

> **A sub-claim withdrawn, and how.** An earlier draft of this section inferred a marginal cost of ~0.8 ns per `u32` from the two `list<u32>` rows (1 element = 800 ns, 1 000 elements = 1 600 ns). A **second measurement run did not reproduce it** — those rows came back 1 400 ns and 900 ns, the opposite order — so at this scale the fixed crossing cost and the marginal copy cost are both ~800 ns and one run cannot separate them. The per-element figure is **not established** and is not published here. Both runs, and the withdrawal, are in [`docs/abi-cost-measured.md` §2 and §3](docs/abi-cost-measured.md).
...
> **Not yet satisfied:** §9.1 requires "three repetitions **with variance**". **Two runs** of 20 000 samples were taken and both are published, but no `Repetitions` spread is published — `Repetitions::new` requires three and refuses fewer, correctly, which is why none appears. What the second run established is that **p99 reproduces across runs to within ~3 % while p50 does not**, and that one sub-claim did not survive. This is recorded as the largest gap in the measured document rather than papered over by listing uncorrelated runs as if they were repetitions.
