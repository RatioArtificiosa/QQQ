...
- [ ] **SRV-020** Implement request-body size and count limits enforced per tenant, with metrics.
  ...
  → **Measured**: 18 limit tests + 6 conversion tests + 2 fault injections, both reproducing; the round-trip test now carries a populated `limits` because the point of a round-trip is that every field survives. Workspace **2122 passed, 0 failed**.
  → **Still not applied**: `serve_connection` does not yet **consult** the built limiter, so a manifest's limits are constructed and unused. `SRV-020` stays open until a request is actually refused by one.
