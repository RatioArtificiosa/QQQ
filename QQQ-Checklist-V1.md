...
- [x] **PERF-001** Build the benchmark harness with the full published methodology.
  → §9.1 The honest benchmark position
  → Done: **`crates/qqq-bench`** — a new workspace crate, second in the topology order because `qqq-core` is its only dependency. It defines what a *measurement* is and nothing that runs, so it can measure any crate above it; a harness that depended on the server could not be used to time the server's own startup.
